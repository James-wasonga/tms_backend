...
// Helper function to safely parse and verify JWT tokens
function parseToken(token) {
  try {
    if (!token) return null;
    // Extract token string if prefixed with "Bearer "
...
exports.requireUserSignin = async (req, res, next) => {
  ...
  if (token) {
    ...
    if (user && user._id) {
      const founduser = await User.findById(user._id).select("name email phone address");
  ...
  return res.status(401).json({ status: "FAILED", error: "Not authorized!" });
};
...
exports.refreshToken = async (req, res) => {
  if (req.body && req.body.token) {
    const parsed = parseToken(req.body.token);
  ...
  return res.status(400).json({ status: "FAILED", error: "Invalid content" });
};
